...
export class ProjectAPI extends BaseAPI {
    ...
    async addItemV4(project, images) {
        ...
        images.forEach((image) => {
            ...
            json_images.push({
                "uuid": image_uuid,
                "project_uuid": project.uuid,
                "blob_original": image.source_original.blob,
                "blob_medium": image.source_medium.blob,
                "blob_medium_modified": null,
                "blob_thumb": image.source_medium.blob,
                "blob_thumb_modified": null,
                "width": image.source_medium.width,
                "height": image.source_medium.height,
                "quality": image.quality,
    ...
    async addItemV5(project, images) {
        const json_images = [];
        const json_images_light = [];
        console.log("entrou no v5", images);
        images.forEach((image) => {

            const _blobImage = new Blob([image], {type: image.type});
            console.log("_blobImage", _blobImage.width);
            const image_uuid = uuidv4();
            json_images_light.push({
                "uuid": image_uuid,
                "width": null,
