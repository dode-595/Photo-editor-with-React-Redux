...
const template = {
    ...
    "paper_types": [
        {
            'type': 1,
            'title': 'Papel Crytal Brilho',
            'subtitle': 'Detalhes aqui',
            'price': '0,15'
...
const template_quadro = {
    "status": 1,
    "title": "Quadro Premium",
    "description": "Descriçao do produto",
    "width": 900,
    "height": 1200,
    "margin": 0,
    ...
    "spine": "Meu novo quadro",
    "containers": [],
    "templates": [
        {
            "categorie": 'Fotos e texto',
            "items": [
                {
                    "name": "2 fotos e 1 texto",
                    "thumb": "/template.jpg",
                    "template": [
                        {
                            "width": 380,
...
async function init_product() {
    ...
    let product_count = await productAPI.listActive();
    let count = await product_count.count();
    ...
    let product2 = await productAPI.new(new_product2, template_quadro).catch((exception) => {
        console.log("exception new", exception);
    });
